...
function MatchCard({ match, onAccept, onDecline, onCancel, busy }) {
  const badge = STATUS_BADGES[match.status] || {
    label: match.status,
    color: "bg-slate-100 text-slate-700",
  };
  const showAcceptDecline = match.status === "pending" && match.requiresResponse && match.isReceivingOwner;
  ...
  function DogInfoCard({ dog, title, isPartner, partnerId }) {
    if (!dog) return <div className="rounded-lg bg-slate-50 p-2 text-xs text-slate-400">No info</div>;
    const cardContent = (
      <div className="rounded-lg bg-slate-50 p-2 flex gap-2 items-center">
        {dog.image_url && (
          <img src={dog.image_url} alt={dog.name} className="w-12 h-12 rounded-full object-cover border border-slate-200" />
        )}
        <div>
          <div className="text-xs text-slate-400 mb-1">{title}</div>
          <div className="text-sm font-semibold text-slate-800">{dog.name}</div>
          <div className="text-xs text-slate-400">{dog.breed || "Unknown"} • {dog.gender || "Unknown"}</div>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        <div>
          <h3 className="text-base font-medium text-slate-900 mb-1">
            {match.myDog?.name} <span className="mx-1 text-slate-400">↔</span> {match.partnerDog?.name || "Owner pending"}
          </h3>
          <div className="text-xs text-slate-400">
            {match.direction === "sent" ? "Sent" : "Received"} · {requestedDate ? requestedDate.toLocaleDateString() : "—"}
          </div>
        </div>
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${badge.color} bg-opacity-60`}>
          {badge.label}
        </span>
      </div>
      <div className="grid grid-cols-2 gap-2 mt-2">
        <DogInfoCard dog={match.myDog} title="Your dog" />
        <DogInfoCard dog={match.partnerDog} title="Partner" isPartner={true} partnerId={match.partnerDog?.id} />
      </div>
      {/* Removed separate View partner profile link */}
      {match.outcome && (
        <div className="mt-3 rounded-lg bg-emerald-50 p-2 text-xs text-emerald-700">
          Outcome: <span className="font-medium">{match.outcome.outcome.replace("_", " ")}</span> {match.outcome.notes && `· ${match.outcome.notes}`}
        </div>
      )}
      <div className="mt-3 flex gap-2">
        {showAcceptDecline && (
          <>
            <button className="px-3 py-1 rounded-full bg-emerald-500 text-white text-xs font-medium transition hover:bg-emerald-600 disabled:opacity-50" onClick={() => onAccept(match)} disabled={busy}>Accept</button>
            <button className="px-3 py-1 rounded-full bg-rose-500 text-white text-xs font-medium transition hover:bg-rose-600 disabled:opacity-50" onClick={() => onDecline(match)} disabled={busy}>Decline</button>
          </>
        )}
        {showCancel && (
          <button className="px-3 py-1 rounded-full border border-slate-300 text-xs font-medium transition hover:bg-slate-100 disabled:opacity-50" onClick={() => onCancel(match)} disabled={busy}>Cancel</button>
        )}
      </div>
    </div>
  );
...
export default function MyMatches({ userId }) {
  ...
  const visibleMatches = useMemo(() => {
    ...
    function isReceivingOwner(match) {
      if (match.direction === 'received') {
        return match.myDog && match.myDog.owner_id === userId;
      } else {
        return match.partnerDog && match.partnerDog.owner_id === userId;
      }
    }
    function isFemaleDogOwner(match) {
      if (match.partnerDog && match.partnerDog.gender === 'female' && match.partnerDog.owner_id === userId) {
        return true;
      }
      if (match.myDog && match.myDog.gender === 'female' && match.myDog.owner_id === userId) {
        return true;
      }
      return false;
    }
    ...
    switch (tab) {
      case "pending":
        ...
        break;
      case "accepted":
        list = matches.filter(m => m.status === "accepted");
        break;
      case "declined":
  ...
  }, [tab, page, pendingMatches.length, awaitingConfirmationMatches.length, historyMatches.length, matches.length]);
    ...
    <div className="find-match-container">
      <div className="header-section">
        ...
        <p className="page-description">
          Track requests, confirmations, and outcomes
        </p>
      </div>
                ...
                <div className="grid gap-6">
                  {visibleMatches.map((match) => (
                    <MatchCard
                      key={match.id}
                      match={match}
                      onAccept={(m) => handleStatusChange(m, "accepted")}
